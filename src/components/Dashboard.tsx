...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    setLoading(true);
    try {
      // 1. Obtener Ingresos Reales (Sumando historial_pagos)
      const { data: pagos } = await supabase.from("historial_pagos").select("monto_abonado");
      const ingresos = pagos?.reduce((acc, curr) => acc + (Number(curr.monto_abonado) || 0), 0) || 0;
      ...
      // 4. Órdenes Activas (Todo lo que NO esté FINALIZADO)
      const { count: countOrders } = await supabase
        .from("service_orders")
        .select("*", { count: 'exact', head: true })
        .neq("status", "FINALIZADO"); // Cambiado para que incluya PENDIENTE y EN PROCESO
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-[2rem] hover:border-emerald-500/50 transition-all group shadow-2xl">
          ...
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Ingresos Reales</p>
          <h2 className="text-3xl font-black text-white mt-1 font-mono">${stats.ingresos.toLocaleString()}</h2>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-6 rounded-[2rem] hover:border-red-500/50 transition-all group shadow-2xl">
          <div className="flex justify-between items-start mb-4">
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-[2rem] hover:border-blue-500/50 transition-all group shadow-2xl">
          <div className="flex justify-between items-start mb-4">
            <div className="p-3 bg-blue-500/10 rounded-2xl text-blue-500">
              <Users size={24} />
            </div>
          </div>
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Clientes</p>
        ...
        <div className="bg-slate-800 border-2 border-orange-500/20 p-6 rounded-[2rem] shadow-xl shadow-orange-500/5 group">
          ...
          <p className="text-orange-500 text-[10px] font-black uppercase tracking-widest">Órdenes Activas</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-10">
            <div className="space-y-3">
              <div className="flex justify-between text-[10px] font-black uppercase tracking-widest">
                <span className="text-slate-400">Gastos vs Rentabilidad</span>
                <span className={`${stats.margen >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>{stats.margen}% Margen</span>
              </div>
              <div className="h-16 w-full bg-slate-800 rounded-2xl overflow-hidden flex p-1 border border-slate-700">
                <div 
                  ...
                  className="bg-red-500/80 rounded-xl flex items-center justify-center text-[10px] font-black text-white italic transition-all duration-1000 min-w-[40px]"
                >
                  Gastos
                </div>
                <div 
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-6 bg-slate-800/50 rounded-2xl border border-slate-700">
                <p className="text-[10px] text-slate-500 font-black uppercase">Balance Neto</p>
                <p className={`text-2xl font-black ${stats.utilidad >= 0 ? 'text-emerald-400' : 'text-red-500'}`}>
                  ${stats.utilidad.toLocaleString()}
                </p>
              </div>
              <div className="p-6 bg-slate-800/50 rounded-2xl border border-slate-700">
                <p className="text-[10px] text-slate-500 font-black uppercase">Vehículos en Base</p>
                <p className="text-2xl font-black text-white">{stats.totalVehiculos}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-[3rem] flex flex-col">
          <h3 className="text-white font-black uppercase italic mb-6 text-sm tracking-widest">Notificaciones</h3>
          <div className="space-y-4 flex-1">
            {stats.ordenesActivas > 0 ? (
              <div className="flex gap-4 p-4 bg-orange-500/10 border border-orange-500/20 rounded-2xl">
                ...
                <p className="text-xs text-slate-300 font-medium">Hay {stats.ordenesActivas} servicios en curso esperando ser finalizados.</p>
              </div>
            ) : (
              <div className="flex gap-4 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl">
                <ClipboardList className="text-emerald-500 shrink-0" />
                <p className="text-xs text-slate-300 font-medium">No hay órdenes pendientes. ¡Taller al día!</p>
              </div>
            )}
            
            <div className="flex gap-4 p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl">
              <Car className="text-blue-500 shrink-0" />
              <p className="text-xs text-slate-300 font-medium">Se han registrado {stats.totalVehiculos} vehículos hasta la fecha.</p>
            </div>
          </div>
          
          <button 
            onClick={() => window.location.href = '/quotes'}
            className="w-full mt-6 bg-orange-600 hover:bg-orange-500 text-white font-black py-4 rounded-2xl transition-all text-[10px] uppercase tracking-widest shadow-lg shadow-orange-600/20"
          >
            Nuevo Presupuesto
          </button>
        </div>
